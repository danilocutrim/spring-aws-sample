...
interface ItemConverter<T, R> {

    fun toSchema(item: T): R
    fun fromSchema(item: R): T
}

sealed class Converter<T, R> : ItemConverter<T, R> {
    override fun toSchema(item: T): R {
        throw NotImplementedError("This method should be implemented by the subclass")
    }

    override fun fromSchema(item: R): T {
        throw NotImplementedError("This method should be implemented by the subclass")
    }
}

class UserConverter : Converter<User, UserSchema>() {
    override fun toSchema(item: User): UserSchema {
        return UserSchema(userName = item.userName, email = item.email, login = item.login)
    }

    override fun fromSchema(item: UserSchema): User {
        val name = requireNotNull(item.userName)
        return User(name, item.email, item.login)
    }
}
...
fun main(){
    val a = DataClassMapper<User, UserSchema>()
    val b = a(User("name", "email", "login"))
    println(b)
}